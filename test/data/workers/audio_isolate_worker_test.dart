...
Uint8List buildPcmBuffer(double freqHz, {double amplitude = 0.8}) {
  ...
  for (int i = 0; i < n; i++) {
    final sample = (amplitude * sin(2 * pi * freqHz * i / sr) * 32767)
        .round()
        .clamp(-32768, 32767);
...
void main() {
  ...
  group('AudioIsolateWorker — détection de pitch', () {
    test(
      'retourne un PitchResult pour A4 (440 Hz) avec confiance ≥ 0.85',
      () async {
        final (:workerPort, :responses) = await spawnWorker();

        final pcm = buildPcmBuffer(440.0);
        workerPort.send(
          AudioBufferMessage(TransferableTypedData.fromList([pcm])),
        );
        ...
        expect(
          result.confidence,
          greaterThanOrEqualTo(AudioConstants.minConfidence),
        );
      ...
      workerPort.send(
        AudioBufferMessage(
          TransferableTypedData.fromList([
            Uint8List(AudioConstants.bufferSize * 2),
          ]),
        ),
      );
  ...
  group('AudioIsolateWorker — mise à jour de config', () {
    test(
      'UpdateConfigMessage modifie la config sans relancer le worker',
      () async {
        final (:workerPort, :responses) = await spawnWorker();
